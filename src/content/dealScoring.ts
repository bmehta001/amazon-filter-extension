...
export function computeDealScore(
  ...
  const { price, listPrice, coupon, hasDealBadge, reviewCount, reviewQuality, subscribeAndSave } = product;
  ...
  // Compute positive deal signals (Factors 1-4)
  const { discountPercent, couponPercent, snsPercent } = scorePositiveSignals(
    product, addSignal, fmt,
  );

  // Detect suspicious patterns (Factors 5-7)
  scoreSuspiciousPatterns(
    { price, listPrice, coupon, hasDealBadge, reviewCount, reviewQuality, discountPercent, couponPercent },
    addSignal, addManipulation, fmt,
  );

  // Cross-reference with price history (Factor 8)
  if (priceHistory && price != null) {
    scoreHistoricalManipulation(price, listPrice, priceHistory, addManipulation, fmt);
  }
...
type AddSignalFn = (type: DealSignal["type"], description: string, points: number) => void;
type AddManipulationFn = (description: string, warning: string, penalty: number) => void;
type FmtFn = (v: number) => string;

/**
 * Factors 1-4: Discount, coupon, S&S, deal badge, and review trust.
 * Returns the computed percentage values needed by later factors.
 */
function scorePositiveSignals(
  product: Product,
  addSignal: AddSignalFn,
  fmt: FmtFn,
): { discountPercent: number; couponPercent: number; snsPercent: number } {
  const { price, listPrice, coupon, hasDealBadge, subscribeAndSave, reviewQuality } = product;
  ...
  return { discountPercent, couponPercent, snsPercent };
}

/**
 * Factors 5-7: Suspicious discount patterns, inflated "Was" price, coupon padding.
 */
function scoreSuspiciousPatterns(
  ctx: {
    ...
    reviewCount: number;
    reviewQuality: number | null | undefined;
    discountPercent: number;
    couponPercent: number;
  },
  addSignal: AddSignalFn,
  addManipulation: AddManipulationFn,
  fmt: FmtFn,
): void {
  const { price, listPrice, coupon, hasDealBadge, reviewCount, reviewQuality, discountPercent, couponPercent } = ctx;
...
/**
 * Factor 8: Cross-reference deal claims against watchlist price history.
 */
function scoreHistoricalManipulation(
  price: number,
  listPrice: number | null | undefined,
  priceHistory: PriceHistoryContext,
  addManipulation: AddManipulationFn,
